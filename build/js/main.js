...
// Function to get the right 100vh
function appHeight() {
...
$(document).ready(function() {
	...
	$('.agree-checkbox').each(function() {
		...
		var agreeCheckboxesCount = thisForm.find('.agree-checkbox').length;

		$(thisCheckbox).on('change', function() {
			if (thisForm.find($(".agree-checkbox:checkbox:checked")).length !== agreeCheckboxesCount) {
	...
	$('#section-program-show-more').click(function () {
		$("#section-program-row").removeClass('is-hidden');
		$("#section-program-footer").hide();
	});
	...
	$('input[type="file"]').on('change', function (e) {
		var parent = $(this).closest('.custom-file-upload');
		var filename = $(this).val().replace(/.*(\/|\\)/, '');
		console.log(filename);

		if (filename) {
			parent.addClass('is-file-added');
			parent.find('[data-upload-name]').html(filename);
		} else {
			parent.removeClass('is-file-added');
		}
	});

	$('[data-upload-clear]').on('click', function (e) {
		var parent = $(this).closest('.custom-file-upload');
		parent.removeClass('is-file-added');
		parent.find('[data-upload-name]').html('');
